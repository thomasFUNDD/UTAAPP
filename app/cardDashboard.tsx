...
const SendScreen = () => {
  ...
  const fetchUnusedVouchers = async () => {
    setIsLoading(true);
    setError(null);
    console.log('fetchUnusedVouchers is called');
    ...
    try {
      ...
      if (token) {
        ...
        console.log(response.data.book_categories);
        setBookCategories(response.data.book_categories);
      } else {
          ...
          {bookCategories.length > 0 ? (
            bookCategories.map((item, index) => (
              <View key={index} style={styles.card}>
                ...
                <Text style={styles.cardText}>ID: {item.bt_id}</Text>
                <Text style={styles.cardText}>Value: {item.bt_value}</Text>
                <Text style={styles.cardText}>Count: {item.count}</Text>
                <Text style={styles.cardText}>Subtotal: £{item.subtotal}</Text>
              </View>
            ))
          ) : (
