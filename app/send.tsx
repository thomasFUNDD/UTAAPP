...
const [uniqueCharities, setUniqueCharities] = useState([]);
  ...
  useEffect(() => {
    // Assuming charities data is fetched and set elsewhere in the component
    const nonPrivateCharities = charities.filter(charity => charity.regchar?.toLowerCase() !== "private");
    const uniqueCharityNames = Array.from(new Set(nonPrivateCharities.map(charity => charity.charity)));
    const uniqueCharitiesResult = uniqueCharityNames.map(name => nonPrivateCharities.find(charity => charity.charity === name));
    setUniqueCharities(uniqueCharitiesResult);
  }, [charities]);

  useEffect(() => {
    if (filteredBankAccounts.length === 1) {
      setSelectedBankAccount(filteredBankAccounts[0].bnk_account_no);
    }
...
// Add these styles to
  ...
  const handleSearch = () => {
      ...
      if (!searchQuery.trim()) {
        setFilteredCharities(uniqueCharities);
      } else {
        const filtered = uniqueCharities.filter((charity) =>
          charity.charity?.toLowerCase().includes(searchQuery.toLowerCase())
        );
...
const togglePaymentReference = () => setShowPaymentReference(previousState => !previousState);
  ...
  const contactSupport = () => {
    ...
    navigation.navigate('changeemail'); // The route name must match exactly // Make sure 'ContactSupport' is correctly set up in your navigator
  };
        ...
        <View style={styles.modalView}>
          {filteredBankAccounts.length > 0 && filteredBankAccounts.some(account => account.bnk_sort_code && account.bnk_account_no) ? (
            ...
            <View style={{alignItems: 'center', padding: 20}}>
              ...
              <TouchableOpacity
                onPress={contactSupport} // Ensure contactSupport is correctly implemented to navigate
                style={{
...
£{typeof currentBalance === 'number' ? currentBalance.toFixed(2) : '0.00'}
  ...
  const sendDonation = async () => {
    // Assuming `inputValue` is the state for the TextInput value
    // Convert inputValue to a number and ensure it's not NaN and greater than 0
    ...
    const requestBody = {
        ...
        amount: donationAmount.toFixed(2).toString(), // Use the validated donationAmount
        anonymous: false,
      ...
      return (
        ...
        >
          <View style={{ /* Modal styling */ }}>
            ...
            />
            <FlatList
              data={filteredCharities.reduce((unique, item) => {
                if (!unique.some(obj => obj.charity === item.charity)) {
                  unique.push(item);
                }
                return unique;
              }, [])}
...
console.log('Selected Bank Account:', selectedBankAccount);
  return (
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.primary }}>
          ...
          {(selectedCharity && selectedBankAccount) && (
...
<TouchableOpacity onPress={() => setDonationEditVisible(!donationEditVisible)}>
  {donationEditVisible ? (
    ...
    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: '100%', paddingBottom: 30 }}>
...
{renderErrorModal()}
            ...
            {(!selectedCharity || !selectedBankAccount) && (
<View style={{ paddingBottom: 10 }}>
  <Text style={{ fontSize: 18, color: COLORS.black, fontWeight: 'bold', marginTop: 20 }}>Charity</Text>
  <TouchableOpacity onPress={() => setCharityModalVisible(true)} style={[styles.bankDetailsButton, styles.whiteBackgroundButton]}>
    <View style={styles.buttonContentContainer}>
...
/>
  ...
  <Text style={{ fontSize: 18, color: COLORS.black, fontWeight: 'bold', marginTop: 20 }}>Bank Account</Text>
  <TouchableOpacity onPress={() => setBankDetailsModalVisible(true)} style={[styles.bankDetailsButton, styles.whiteBackgroundButton]}>
    <View style={styles.buttonContentContainer}>
      <Text style={{ fontSize: 18, color: COLORS.black }}>
        {selectedBankAccount ? selectedBankAccount: "Choose Bank Account"}
      </Text>
    </View>
  </TouchableOpacity>
</View>
...
<Switch
  onValueChange={togglePaymentReference}
  value={showPaymentReference}
  trackColor={{ false: "#767577", true: COLORS.primary }} // COLORS.primary when the switch is on
  thumbColor={showPaymentReference ? COLORS.primary : "#f4f3f4"} // Use COLORS.primary for the thumb when on
/>
...
<Switch
  ...
  trackColor={{ false: "#767577", true: COLORS.primary }} // COLORS.primary when the switch is on
  thumbColor={showSpecialInstructions ? COLORS.primary : "#f4f3f4"} // Use COLORS.primary for the thumb when on
/>
